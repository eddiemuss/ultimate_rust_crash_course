...
fn main() {
    ...
    pause_ms(1 * 1000); // Make sure Thread A has time to get going before we spawn Thread B
    ...
    wait_and_close_thread(vec![handle_a, handle_b]);
    ...
    let (sender, receiver_1) = channel::unbounded();
    let receiver_2 = receiver_1.clone();

    let handle_c = thread::spawn(move || {
        println!("Begin of thread: C");

        for msg in receiver_1 {
            println!("message in thread c: {msg}");
        }
    });

    let handle_d = thread::spawn(move || {
        println!("Begin of thread: D");

        for msg in receiver_2 {
            println!("message in thread d: {msg}");
        }
    });

    for msg in vec!["hello", "there", "good evening"] {
        pause_ms(1 * 1000);
    ...
    wait_and_close_thread(vec![handle_c, handle_d]);

    println!("Main thread: Exiting.")
}

fn wait_and_close_thread<T>(threads: Vec<JoinHandle<T>>) {
    let _ = threads.into_iter().map(|thread| thread.join().unwrap());
}
